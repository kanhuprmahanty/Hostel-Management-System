...
const Login = () => {
  ...
  const fillCredentials = (role) => {
    ...
    if (role === 'admin') setEmail('admin@hms.com');
    if (role === 'warden') setEmail('warden@hms.com');
    if (role === 'student') setEmail('student@hms.com');
  };
      ...
      <div className="max-w-md w-full bg-white rounded-2xl shadow-xl overflow-hidden">
        ...
        <div className="p-8">
          ...
          <div className="mt-6 text-center">
            <p className="text-sm text-gray-500 mb-2">New to HMS?</p>
            <Link to="/register" className="text-primary-600 hover:text-primary-700 font-medium">Create an account</Link>
          </div>

          <div className="mt-8 pt-6 border-t border-gray-100">
            <p className="text-sm text-gray-500 mb-4 text-center">Demo Accounts (Click to fill)</p>
            <div className="flex justify-center space-x-2">
              <button onClick={() => fillCredentials('admin')} className="px-3 py-1 text-xs font-medium bg-gray-100 text-gray-700 rounded-full hover:bg-gray-200 transition">Admin</button>
              <button onClick={() => fillCredentials('warden')} className="px-3 py-1 text-xs font-medium bg-gray-100 text-gray-700 rounded-full hover:bg-gray-200 transition">Warden</button>
              <button onClick={() => fillCredentials('student')} className="px-3 py-1 text-xs font-medium bg-gray-100 text-gray-700 rounded-full hover:bg-gray-200 transition">Student</button>
            </div>
          </div>
        </div>
      </div>
